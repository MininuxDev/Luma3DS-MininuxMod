...
static void menuDraw(Menu *menu, u32 selected)
{
    ...
    if(miniSocEnabled)
    {
        ...
        Draw_DrawFormattedString(SCREEN_BOT_WIDTH - 10 - SPACING_X * 15, 10, COLOR_WHITE, "%15s", ipBuffer);
    }
    else
    ...
    Draw_DrawFormattedString(10, SCREEN_BOT_HEIGHT - 20, COLOR_TITLE, "Luma3DS (Mininux Mod) %s", versionString);

    if(!isRelease) {
        Draw_DrawFormattedString(10, SCREEN_BOT_HEIGHT - 20, COLOR_TITLE, "Luma3DS (Mininux Mod) %s", versionString);
        Draw_DrawFormattedString(10, SCREEN_BOT_HEIGHT - 10, COLOR_TITLE, "R%08lx", commitHash);
    }
